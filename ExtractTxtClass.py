...
class TextExtractor:
    ...
    def MainFunc(self, fPDFPath, fPDFFilename, fPDFHash) :
        ...
        #  COMMENTED OUT
            ...
            self.Logger(f"brownfield_pdfs/{self.parentFolder}/results/{self.hashedName}/{self.hashedName}_Ident.txt",[self.filename, "  |  " + self.parentFolder, f" | Pages: {self.pagesDoc}", f" | is text: {self.IsTextDoc}"])
            #print(result)
            # Display the content of the page
            result = ''.join(text_per_page[dctkey][0])
            print("page text")
            self.Logger(f"brownfield_pdfs/{self.parentFolder}/results/{self.hashedName}/{self.hashedName}_{dctkey}_text.txt",result)
            #print(result)

            print("formatting")
            result = ''.join(str(v) for v in text_per_page[dctkey][1])
            self.Logger(f"brownfield_pdfs/{self.parentFolder}/results/{self.hashedName}/{self.hashedName}_{dctkey}_formatting.txt",result)
            #print(text_per_page[dctkey][1])

            print("text image")
            result = ''.join(text_per_page[dctkey][2])
            self.Logger(f"brownfield_pdfs/{self.parentFolder}/results/{self.hashedName}/{self.hashedName}_{dctkey}_image.txt",result)
            #print(result)

            print("text tables")
            result = ''.join(text_per_page[dctkey][3])
            self.Logger(f"brownfield_pdfs/{self.parentFolder}/results/{self.hashedName}/{self.hashedName}_{dctkey}_tables.txt",result)
            #print(result)

            print("page content")
            result = ''.join(text_per_page[dctkey][4])
            self.Logger(f"brownfield_pdfs/{self.parentFolder}/results/{self.hashedName}/{self.hashedName}_{dctkey}_content.txt",result)
            #print(result)

            #finally create a wordmap of the page contents
            print("creating wordmap")
            self.count_word_occurrences_with_locations(f"brownfield_pdfs/{self.parentFolder}/results/{self.hashedName}/{self.hashedName}_{dctkey}_content.txt", f"brownfield_pdfs/{self.parentFolder}/results/{self.hashedName}/{self.hashedName}_{dctkey}_wordmap.txt")
    ...
    def ProcessExtractPDFs(self) -> bool:
        ...
        for document in self.all_files :
                ...
                self.MainFunc(('brownfield_pdfs/' + document.folderSource + '/'), document.fileName, document.hashValue)
        try:
